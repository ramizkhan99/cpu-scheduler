/* test_process.c: Test CSE Process */
...
int test_01_process_start() {
    ...
    char test[BUFSIZ];
    snprintf(test, BUFSIZ, "test -n \"$(ps ux | awk '$2 == %d {print $2}')\"",
             p->pid);
...
int test_02_process_pause() {
    ...
    char test[BUFSIZ];
    snprintf(test, BUFSIZ,
             "test \"$(ps ux | awk '$2 == %d {print substr($8, 0, 1)}')\" = T",
             p->pid);
    assert(system(test) == 0);
    free(p);
    return EXIT_SUCCESS;
...
int test_03_process_resume() {
    ...
    char test[BUFSIZ];
    snprintf(test, BUFSIZ,
             "test \"$(ps ux | awk '$2 == %d {print substr($8, 0, 1)}')\" = S",
             p->pid);
    assert(system(test) == 0);
    free(p);
    return EXIT_SUCCESS;
...
int main(int argc, char *argv[]) {
    ...
    switch (number) {
    case 0:
        status = test_00_process_create();
        break;
    case 1:
        status = test_01_process_start();
        break;
    case 2:
        status = test_02_process_pause();
        break;
    case 3:
        status = test_03_process_resume();
        break;
    default:
